...
def find_obstruction_positions(map, start_position, start_direction)
  reachable_positions = compute_reachable_positions(map, start_position, start_direction)
  valid_positions = []

  reachable_positions.each do |position|
    next if position == start_position # Exclude guard's starting position
    next unless map[position[0]][position[1]] == '.' # Consider only empty spaces

    # Temporarily place obstruction and simulate
    map[position[0]][position[1]] = '#'
    loop_detected = simulate_patrol(map, start_position, start_direction)
    valid_positions << position if loop_detected
    map[position[0]][position[1]] = '.' # Remove obstruction
  end
